...
(defvar hexl-mode-map
  (let ((map (make-keymap)))
    ...
    ;; remacs: `keymapp' does not resolve a symbol's function cell;
    ;; use the `esc-map' variable directly (same object).
    (fset 'hexl-ESC-prefix (copy-keymap esc-map))
    ...
    ;; remacs: `keymapp' does not resolve a symbol's function cell;
    ;; use the `ctl-x-map' variable directly (same object).
    (fset 'hexl-C-x-prefix (copy-keymap ctl-x-map))
