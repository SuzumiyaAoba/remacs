...
;; Ported from GNU Emacs view.el for remacs.

;;; Commentary:

;; This package provides the `view' minor mode loaded by GNU Emacs's
;; `view-mode', `view-buffer', `view-file' and friends: a pager-style
;; read-only browsing mode with less-like scrolling and searching
;; commands.
...
the F command in view-mode, but you can set it to non-nil permanently
if you want that behavior for all buffers (though, that isn't likely
...
Commands \\[View-leave], \\[View-quit] and \\[View-kill-and-leave] will
then restore all windows displaying the buffer, not only the selected
one."
  :type 'boolean
  :group 'view)

(defcustom view-inhibit-help-message nil
  "Non-nil inhibits the `View mode' message when entering View mode."
  :type 'boolean
  :group 'view
  :version "28.1")
...
;; (defvar view-return-alist nil
;;   "(buffer . return-info) alist of buffers to return to.
;; The car of each element is a buffer that was viewed.
;; The cdr of each element is a list (old-buffer pos*3/[window start pos]
;; or (window window . edges)).
;; pos*3/[window start pos] is a list like
;; ((old-point new-point new-beg new-end old-beg old-end) ...)
;; where new-* refer to positions when returning to old-buffer.
;; [window start pos] means that old-buffer was in WINDOW,
;; with start position START and point at position POS.
;; The format of the second is ((old-point new-point)
;; (window start pos old-beg old-end) ...
;; [window window . edges] is a list of one window and window edges
;; corresponding to the whole buffer and the previous buffer,
;; respectively.")

;; (defvar view-last-regexp nil)

;; (defvar view-last-command nil)
;; (defvar view-last-command-entry nil)
;; (defvar view-last-command-argument nil)

(defvar view-exit-action nil
  "If non-nil, `view-mode-exit' calls this function to bury or kill a viewed buffer.
The function is called with the viewed buffer as argument.
...
Emacs commands editing the buffer contents are not available; instead, a
special set of commands (mostly letters and punctuation)
...
;; Buffer-local vars used by the mode machinery.
(defvar-local view-page-size nil)
(defvar-local view-half-page-size nil)
(defvar-local view-old-buffer-read-only nil)
(defvar-local view-last-regexp nil)
(defvar-local read-only-mode--state nil)
