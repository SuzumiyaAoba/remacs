...
(defvar cl-struct-cl--random-state-tags '(cl--random-state))

(defun cl--random-state-i--inliner (inline--form ob)
  (ignore inline--form)
  `(progn (or (cl-random-state-p ,ob)
              (signal 'wrong-type-argument
                      (list 'cl--random-state ,ob)))
          (aref ,ob 1)))
(function-put 'cl--random-state-i 'compiler-macro
              'cl--random-state-i--inliner)
(defun cl--random-state-j--inliner (inline--form ob)
  (ignore inline--form)
  `(progn (or (cl-random-state-p ,ob)
              (signal 'wrong-type-argument
                      (list 'cl--random-state ,ob)))
          (aref ,ob 2)))
(function-put 'cl--random-state-j 'compiler-macro
              'cl--random-state-j--inliner)
(defun cl--random-state-vec--inliner (inline--form ob)
  (ignore inline--form)
  `(progn (or (cl-random-state-p ,ob)
              (signal 'wrong-type-argument
                      (list 'cl--random-state ,ob)))
          (aref ,ob 3)))
(function-put 'cl--random-state-vec 'compiler-macro
              'cl--random-state-vec--inliner)
(defun cl-random-state-p--inliner (inline--form ob)
  (ignore inline--form)
  `(and (memq (type-of ,ob) cl-struct-cl--random-state-tags) t))
(function-put 'cl-random-state-p 'compiler-macro
              'cl-random-state-p--inliner)
(defun cl--make-random-state--cmacro (inline--form vec)
  (ignore inline--form)
  `(let* ((vec ,vec))
     (cl-block cl--make-random-state
       (record 'cl--random-state -1 30 vec))))
(function-put 'cl--make-random-state 'compiler-macro
              'cl--make-random-state--cmacro)
(mapc (lambda (s) (fmakunbound (intern (format "(setf %s)" s))))
      '(cl--random-state-i cl--random-state-j cl--random-state-vec))
