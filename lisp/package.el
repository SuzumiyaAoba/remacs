...
;; remacs: `cl-defmacro' does not support `&rest VAR &key' combined
;; destructuring, so the keywords are extracted from BODY manually
;; (callers always pass the leading keyword pairs first, as GNU does).
(defmacro package--with-response-buffer (url &rest body)
...
\(fn URL &key ASYNC FILE ERROR-FORM NOERROR &rest BODY)"
  ...
  (let ((async (plist-get body :async))
        (file (plist-get body :file))
        (error-form (plist-get body :error-form))
        (noerror (plist-get body :noerror)))
