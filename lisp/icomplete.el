...
(defun icomplete--fido-ccd ()
  ...
  ;; Written without `cl-loop' (destructured `for =' unsupported).
  (mapcar
   (lambda (cat.alist)
     (cons (car cat.alist)
           (mapcar
            (lambda (entry)
              (let ((prop (car entry)) (val (cdr entry)))
                (if (and (eq prop 'styles)
                         ;; Never step in front of 'external', as that
                         ;; might lose us completions.
                         (not (memq 'external val)))
                    (cons prop (cons 'flex (delq 'flex val)))
                  entry)))
            (cdr cat.alist))))
...
(defun icomplete--render-vertical (comps md)
  (let* (scroll-above scroll-below
  ...
  ;; positions.
  ;; Written without `cl-loop' (`collect into'/`concat' unsupported).
  (let* ((preds icomplete--scrolled-past)
         (succs (cdr comps))
         (half-count
          (let ((cnt 0) (cs comps) (left (truncate total-space 2)))
            (while (and (> left 0) (listp (cdr cs)))
              (setq cnt (1+ cnt) left (1- left) cs (cdr cs)))
            cnt))
         (space-above (- total-space 1 half-count))
         (iters total-space)
         (neighbor nil)
         (scroll-below-aux nil))
    (while (and (> iters 0)
                (progn
                  (setq neighbor nil)
                  (if (and preds (> space-above 0))
                      (progn
                        (push (setq neighbor (pop preds)) scroll-above)
                        (setq space-above (1- space-above)))
                    (when (consp succs)
                      (push (setq neighbor (pop succs)) scroll-below-aux)))
                  neighbor))
      (setq iters (1- iters)))
    (setq scroll-below (nreverse scroll-below-aux)))
  ;; Halfway there...
  (let* ((selected (propertize (car comps) 'icomplete-selected t))
    ...
    ;; (Written without `cl-loop': `maximizing into'/`collect into'
    ;; accumulators are unsupported.)
    (let (mpl mcl)
      (dolist (tup tuples)
        (setq mpl (max (or mpl 0) (length (cadr tup)))
              mcl (max (or mcl 0) (length (car tup)))))
      (setq max-prefix-len mpl
            max-comp-len mcl))
    ;; Serialize completions and section titles into a list
    ;; of lines to render
    (let ((lines-aux nil) (nsec 0))
      (dolist (tup tuples)
        (let* ((comp-no-indicator (car tup))
               (prefix (cadr tup))
               (suffix (caddr tup))
               (section (cadddr tup))
               (comp
                (if icomplete-vertical-render-prefix-indicator
                  ...
                  comp-no-indicator)))
          (when section
            (push (propertize section 'face 'icomplete-section) lines-aux)
            (setq nsec (1+ nsec)))
          (when (get-text-property 0 'icomplete-selected comp)
            (add-face-text-property 0 (length comp)
          ...
          (push (concat prefix
                        ...
                        suffix)
                lines-aux)))
      (setq lines (nreverse lines-aux)
            nsections (and (> nsec 0) nsec)))
    ...
    (concat " \n"
            ;; Written without `cl-loop' (`concat' action unsupported).
            (let ((out "") (left total-space))
              (dolist (l lines)
                (when (> left 0)
