...
;; Port of GNU Emacs lisp/emacs-lisp/gv.el (Emacs 31).
     ...
     ,(cond
       ((and (eq symbol 'gv-expander)
        ...
        `(gv-define-expander ,name
           (lambda (,(caadr handler) ,@args) ,@(cddr handler))))
       ((and (eq symbol 'gv-expander) (symbolp handler))
        `(gv-define-expander ,name #',handler))
       ((and (eq symbol 'gv-setter)
             (consp handler)
             (eq (car handler) 'lambda)
             (consp (cadr handler))
             (null (cdadr handler)))
         ;; (gv-setter . (lambda (store) . body))
        `(gv-define-setter ,name (,(caadr handler) ,@args) ,@(cddr handler)))
       ((and (eq symbol 'gv-setter) (symbolp handler))
        `(gv-define-simple-setter ,name ,handler ,fix))
       (t (message "Unknown %s declaration %S" symbol handler) nil))))
...
;;;###autoload
(put 'gv-place 'edebug-elem-spec '(form))
...
generalized variables in place forms."
  (ignore setter)
  getter)
;; GNU attaches these via `(declare (gv-expander funcall) (compiler-macro
;; ...))' inside the defun; our `declare' is inert, so register them
;; directly.  GNU's declare machinery generates a named
;; `gv-synthetic-place--anon-cmacro' for the compiler-macro.
(put 'gv-synthetic-place 'gv-expander #'funcall)
(defun gv-synthetic-place--anon-cmacro (_ getter setter)
  (ignore getter setter)
  getter)
(function-put 'gv-synthetic-place 'compiler-macro
...
This macro only makes sense when used in a place."
  place)
;; GNU: (declare (gv-expander (lambda (do) ...))) inside the defmacro;
;; register the expander directly.
(put 'gv-delay-error 'gv-expander
     (lambda (do place)
       (condition-case err
           (gv-get place do)
         (gv-invalid-place
