...
(defcustom icalendar-vcalendar-prodid
...
(defconst icalendar-vcalendar-version "2.0"
...
(defun icalendar--dmsg (&rest args)
  ...
  (if (or icalendar-debug (= 0 icalendar-debug-level))
      (with-current-buffer (icalendar-error-buffer)
        (goto-char (point-max))
...
;; Error handling
;; ======================================================================
(define-error 'icalendar-error "iCalendar error")

(defconst icalendar-error-buffer-name "*icalendar-errors*"
  "Name of buffer in which errors are listed when processing iCalendar data.")

(defun icalendar-error-buffer ()
  "Return the iCalendar errors buffer, creating it if necessary.
The buffer name is based on `icalendar-error-buffer-name'."
  (get-buffer-create icalendar-error-buffer-name))

(defvar icalendar-inhibit-error-erasure nil
  "When non-nil, `icalendar-init-error-buffer' will not erase the errors
buffer.")

(defun icalendar-init-error-buffer (&optional err-buffer)
...
Erases ERR-BUFFER and places it in `icalendar-errors-mode'."
  (with-current-buffer (or err-buffer (icalendar-error-buffer))
    (unless icalendar-inhibit-error-erasure
      (let ((inhibit-read-only t))
        (erase-buffer)))
    (if (not (eq major-mode 'icalendar-errors-mode))
        (icalendar-errors-mode))))

(defun icalendar-errors-p (&optional err-buffer)
  "Return non-nil if iCalendar errors have been reported in ERR-BUFFER.
ERR-BUFFER defaults to the buffer returned by `icalendar-error-buffer'."
  (with-current-buffer (or err-buffer (icalendar-error-buffer))
    (not (= (point-min) (point-max)))))

(defun icalendar-warn (msg &rest err-plist)
  ...
  (icalendar-handle-generic-error `(icalendar-warning . ,err-plist)))

(defconst icalendar-error-regexp
  (rx line-start
      (zero-or-one
       (group "("
...
(cl-defun icalendar-format-error (&rest error-plist
...
(defun icalendar-handle-generic-error (err-data &optional err-buffer)
...
SIGNAL-DATA is a plist of error data."
  (let* ((signal-data (cdr err-data))
         ...
         (buf (current-buffer)))
    (when (<= icalendar-debug-level severity)
      (with-current-buffer (or err-buffer (icalendar-error-buffer))
        (goto-char (point-max))
        (let ((inhibit-read-only t))
          (unless (bolp) (insert "\n"))
          (insert (apply #'icalendar-format-error
                         (or err-plist
                             (list :buffer buf
                                   :message
...
(defmacro icalendar-condition-case (var bodyform &rest handlers)
...
`icalendar-handle-generic-error'."
  `(condition-case ,var
       ,bodyform
     ,@handlers
     (icalendar-error (icalendar-handle-generic-error ,var))))

;;; Mode based on compilation-mode for navigating error buffer:
(defun icalendar--buffer-from-error ()
  (when-let* ((name (match-string 1)))
    (or (get-buffer name)
        (find-buffer-visiting name))))

(defun icalendar--filename-from-error ()
  (when-let* ((buf (icalendar--buffer-from-error)))
    (buffer-file-name buf)))

(defun icalendar--lineno-from-error ()
  (when-let* ((buf (icalendar--buffer-from-error))
...
(defconst icalendar-error-regexp-alist
  (list (list icalendar-error-regexp
              #'icalendar--filename-from-error
              #'icalendar--lineno-from-error
              nil
              nil
              nil
...
(define-compilation-mode icalendar-errors-mode "iCalendar Errors"
  "Mode for listing and visiting errors when processing iCalendar data."
  (setq-local compilation-error-regexp-alist icalendar-error-regexp-alist))
...
(defun icalendar-make-uid (&optional contents _)
...
The second optional argument is for backward compatibility and is ignored."
  ...
  (let* ((uid icalendar-uid-format)
         ...
         (tohash (or contents
                     (format "%d%s%s" icalendar--uid-count (system-name) timestamp))))
