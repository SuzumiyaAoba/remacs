...
;;; Convenience
;;;
;; remacs: `cl-defmacro' does not accept `&body'; `&rest' is
;; equivalent here since (declare (indent 1)) is already given.
(cl-defmacro jsonrpc-lambda (cl-lambda-list &rest body)
...
dispatcher in CONN."
  ;; remacs: `cl-destructuring-bind' does not accept `&rest' + `&key'
  ;; together; bind WHOLE and its keys with a plain `let*'.
  (let* ((whole (jsonrpc-convert-from-endpoint conn foreign-message))
         (method (plist-get whole :method))
         (id (plist-get whole :id))
         (error (plist-get whole :error))
         (params (plist-get whole :params))
         (result (plist-get whole :result))
         (_jsonrpc (plist-get whole :jsonrpc)))
    (unwind-protect
...
;; remacs: `cl-defmethod' does not accept `&key' with typed
;; specializers; bind the keys from the &rest plist manually.
(cl-defmethod jsonrpc-connection-send ((connection jsonrpc-process-connection)
  ...
  (let ((id (plist-get args :id))
        ...
        (result-supplied-p (plist-member args :result)))
  (when method
