...
;; Port of GNU Emacs 31.1 uniquify.el.
;; Differences from GNU: `eval-when-compile (require 'cl-lib)' dropped —
;; cl-defstruct is preloaded; the rename-buffer/create-file-buffer/kill
;; hooks are wired by the runtime like GNU's buffer.c/files.el calls.
...
;; subset lacks :constructor/:copier options).  Layout: a :named
;; record [uniquify-item base dirname buffer proposed].
(defun uniquify-make-item (base dirname buffer &optional proposed)
  (record 'uniquify-item base dirname buffer proposed))
(defun uniquify-item-p (ob)
  (and (recordp ob) (eq (aref ob 0) 'uniquify-item)))
(defun uniquify-item-base (ob) (aref ob 1))
(defun uniquify-item-dirname (ob) (aref ob 2))
(defun uniquify-item-buffer (ob) (aref ob 3))
(defun uniquify-item-proposed (ob) (aref ob 4))
;; remacs's setf falls back to a `(setf NAME)' function called (FN V OBJ).
(fset (intern "(setf uniquify-item-dirname)")
      (lambda (v ob) (aset ob 2 v)))
(fset (intern "(setf uniquify-item-proposed)")
      (lambda (v ob) (aset ob 4 v)))
