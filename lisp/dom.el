;;; dom.el --- XML/HTML DOM manipulation and searching -*- lexical-binding: t; -*-

;; Ported from GNU Emacs dom.el.  A DOM is a list
;;   (tag attributes . children)
;; as produced by `libxml-parse-xml-region'/`libxml-parse-html-region'.
;; Adaptations: `cl-remove-if-not' is inlined as a negated predicate,
;; `with-work-buffer' is `with-temp-buffer', `define-inline' is
;; `defsubst', `pcase-dolist' is spelled as `dolist', and
;; `url-insert-entities-in-string' is a local string replacement.
...
(defun dom-text (node)
  ...
  (mapconcat #'identity
             (cl-remove-if (lambda (c) (not (stringp c)))
                           (dom-children node))
             " "))
...
A name is a symbol like `td'."
  ;; (GNU writes these loops with cl-loop `for X = ...'; our cl-loop
  ;; subset lacks `for =' so they're spelled with dolist.)
  (let ((matches nil))
    (dolist (child (dom-children dom))
      (let ((m (and (not (stringp child))
		    (dom-by-tag child tag))))
	(when m (setq matches (append matches m)))))
...
(defun dom-strings (dom)
  ...
  ;; (cl-loop `if/else collect/append' scrambles order in our subset;
  ;; spelled with dolist.)
  (let ((out nil))
    (dolist (child (dom-children dom))
      (if (stringp child)
	  (setq out (append out (list child)))
	(setq out (append out (dom-strings child)))))
    out))
...
(defsubst dom--html-boolean-attribute-p (attr)
...
(defun dom--insert-entities (string)
  "Like `url-insert-entities-in-string': escape & < > in STRING."
  (mapconcat
   (lambda (c)
     (cond ((eq c ?&) "&amp;")
           ((eq c ?<) "&lt;")
...
If XML, generate XML instead of HTML."
  (let ((column (current-column))
    ...
    (insert (format "<%s" (dom-tag dom)))
    (dolist (pair (dom-attributes dom))
      (let ((attr (car pair)) (value (cdr pair)))
        ...
        (when value
          (insert
           ...
           (if (and (not xml) (dom--html-boolean-attribute-p attr))
               (format " %s" attr)
             (format " %s=%S" attr (dom--insert-entities
                                    (format "%s" value))))))))
	...
	(insert ">")
        (dolist (child children)
	  (if (stringp child)
	      (insert (dom--insert-entities child))
	    (setq non-text t)
	    (when pretty
              (insert "\n")
