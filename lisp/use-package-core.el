...
(require 'bytecomp nil t)   ; remacs: no byte-compiler; soft-require.
(require 'cl-lib)
...
(defun use-package-keyword-index (keyword)
  ;; remacs: `cl-loop' has no `named' clause; use `cl-block' instead.
  (cl-block outer
    ...
    (let* ((keyword (car input))
           ...
           (normalizer
            (intern-soft (concat "use-package-normalize/"
                                 (symbol-name keyword))))
           ;; remacs: `functionp' does not chase defalias chains
           ;; (use-package-normalize/* are aliases), so use `fboundp'.
           (arg (and (fboundp normalizer)
      ...
      (while iargs
        (when (keywordp (car iargs))
          (let ((autoloads
            ...
            ;; remacs: `functionp' does not chase defalias chains
            ;; (use-package-autoloads/* are aliases), so use `fboundp'.
            (when (fboundp autoloads)
              (setq commands
