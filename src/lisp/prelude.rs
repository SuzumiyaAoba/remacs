...
(fset 'mode-specific-command-prefix
  (let ((s (make-keymap)))
  (define-key s [94] (let ((s (make-keymap)))
...
(fset 'help-command
  ...
  (define-key s [52] (let ((s (make-keymap)))
...
(fset 'vc-prefix-map
  (let ((s (make-keymap)))
  (define-key s [119] (let ((s (make-keymap)))
  ...
  (define-key s [69] (let ((s (make-keymap)))
  ...
  s))
  (define-key s [84] (let ((s (make-keymap)))
  (define-key s [82] (let ((s (make-keymap)))
  ...
  s))
  (define-key s [77] (let ((s (make-keymap)))
  ...
  (define-key s [98] (let ((s (make-keymap)))
...
(fset 'Control-X-prefix
  ...
  (define-key s [13] (let ((s (make-keymap)))
  ...
  (define-key s [27] (let ((s (make-keymap)))
  ...
  (define-key s [56] (let ((s (make-keymap)))
  (define-key s [101] (let ((s (make-keymap)))
  ...
  (define-key s [97] (let ((s (make-keymap)))
  ...
  (define-key s [105] (let ((s (make-keymap)))
  ...
  (define-key s [110] (let ((s (make-keymap)))
  ...
  (define-key s [112] (let ((s (make-keymap)))
  (define-key s [24] (let ((s (make-keymap)))
  ...
  (define-key s [114] (let ((s (make-keymap)))
  ...
  (define-key s [116] (let ((s (make-keymap)))
  ...
  (define-key s [119] (let ((s (make-keymap)))
  (define-key s [102] (let ((s (make-keymap)))
  ...
  (define-key s [111] (let ((s (make-keymap)))
  ...
  s))
  (define-key s [120] (let ((s (make-keymap)))
...
(fset 'ESC-prefix
  ...
  (define-key s [103] (let ((s (make-keymap)))
  ...
  (define-key s [115] (let ((s (make-keymap)))
  ...
  (define-key s [104] (let ((s (make-keymap)))
...
;; Restore GNU's stored keymap order: `define-key' prepends new
;; bindings, but the dump above was emitted in GNU's `map-keymap'
;; iteration order, so each map's spine was built reversed.  Reverse
;; every reachable keymap's own elements (keeping any parent tail)
;; so iteration order matches GNU.
  ...
  (let ((m (cdr e)))
    ...
    (when (and (consp m) (eq (car m) 'keymap)
      ...
      (let ((elts '())
            ...
            (tail nil))
        (while (consp cur)
          (if (eq (car cur) 'keymap)
              (progn (setq tail cur) (setq cur nil))
            (push (car cur) elts)
            (setq cur (cdr cur))))
        ;; `elts' is the spine reversed — nconc it onto the parent tail.
        (setcdr m (nconc elts tail))))))
